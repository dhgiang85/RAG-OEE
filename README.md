...
### Bước 2.5: Khởi tạo Tài Khoản Quản Trị Hệ Thống (Dev / Prod Admin Accounts)

Database Migrations (`00001_init.sql`) chạy ở **Bước 4.2** sẽ tự động nâng cấp cấu trúc cơ sở dữ liệu lên phiên bản mới nhất (**Update HEAD**), đồng thời khởi tạo sẵn **Default Tenant** và **Tài khoản Dev mặc định**:
* **Default Tenant ID**: `00000000-0000-0000-0000-000000000001` (Tên: `default`)
* **Default User ID**: `11111111-1111-1111-1111-111111111111` (Email: `dev@local`)

#### 1. Sử dụng Tài khoản Dev giả lập (Chế độ thử nghiệm):
Trong môi trường thử nghiệm (khi biến `JWT_SECRET` trong tệp `.env` được để trống), Gateway cho phép bypass xác thực bằng cách đính kèm hai Header giả lập sau trong các yêu cầu HTTP API:
* `X-Dev-Tenant-Id: 00000000-0000-0000-0000-000000000001`
* `X-Dev-User-Id: 11111111-1111-1111-1111-111111111111`

#### 2. Khởi tạo tài khoản Admin thực tế (Chế độ Production):
Khi triển khai thực tế trên máy chủ mới với biến `JWT_SECRET` đã được cấu hình bảo mật, cơ chế bypass giả lập trên sẽ bị **vô hiệu hóa**. Vì tài khoản `dev@local` khởi tạo mặc định chỉ chứa mã hash giữ chỗ không thể dùng để đăng nhập, bạn **phải tạo một tài khoản Admin thực tế** có mật khẩu bảo mật riêng qua CLI của container `ops`:

Chạy lệnh sau tại thư mục triển khai trên máy chủ mới để tạo tài khoản:
```bash
docker compose run --rm ops users create \
